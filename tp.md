...
Q3. La fonction de déchiffrement est ```y*x modulo p``` tel que ```a^{p-2} = y modulo p``` et x est un élément de la liste des clés (c'est à dire les images obtenues après le chiffrement en question 2). Déchiffrez votre message chiffré obtenu en question 2.  
